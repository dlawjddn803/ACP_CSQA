...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = outputs[1]
        pooled_output = pooled_output.view(bsz, num_choices, -1, self.config.hidden_size).mean(2).squeeze()
        pooled_output = pooled_output.view(-1, self.config.hidden_size)  # 10 x 768
        pooled_output = self.dropout(pooled_output)

        reshaped_logits = pooled_output.view(-1, num_choices, self.config.hidden_size)  # 2 x 5 x 768
